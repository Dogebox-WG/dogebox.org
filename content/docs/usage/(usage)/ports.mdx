---
title: Forwarding Public Ports
---

Depending on what Pups you install you will likely want to forward inbound connections. For example, the Dogecoin Core Pup requires port `22556` while Dogenet requires port `42069`. Other Pups may also require inbound connection forwarding depending on what they do. 
...
2. Login into the control panel of the router. If it hasn't been changed from the default, You will need to use the username and password provided by the router manufacturer. Sometimes, a sticker is placed on the bottom or the side of the router revealing the admin password. 
