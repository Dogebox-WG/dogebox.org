...
blockchain at a given point in time.
...
Copy the contents of the Blockchain Backup Drive (the `blocks` and `chainstate` folders) into the
Dogecoin Core Pup storage folder `~/data/pups/storage/<dogecoin-pup-id>/`, overwriting any data
already present.
