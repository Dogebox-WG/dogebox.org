...
Device name allows access using mDNS for example `device-name.local:8080` or `device-name:8080` (macOS will use `.local`, linux might not).
Keyboard layout is needed for interacting with the system through ssh, or with a keyboard and monitor attached.
...
Set your password, it's used to encrypt your root key and seed phrase so make sure it's secure!
This password is also used for the Web Admin interface, but not for ssh or commandline login.
...
You may also add an SSH public key to enable SSH access.
